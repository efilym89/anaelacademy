...
function getPresentationPreviewState(lessonId) {
  return presentationPreviewState.get(lessonId) ?? { loading: false, viewerUrl: '', error: '' };
}

function setPresentationPreviewState(lessonId, nextState) {
  const currentState = getPresentationPreviewState(lessonId);
  presentationPreviewState.set(lessonId, {
    ...currentState,
    ...nextState
  });
}

function buildPresentationPanel(lesson) {
  const previewState = getPresentationPreviewState(lesson.id);
  const folder = lesson.storage?.folder ?? 'storage';
  const hasPresentation = Boolean(lesson.presentation?.href);
  const presentationMarkup = hasPresentation
    ? `
        <button class="button" id="openPresentationPreview" type="button" ${previewState.loading ? 'disabled' : ''}>
          ${escapeHtml(previewState.loading ? PRESENTATION_PREVIEW_LOADING_TEXT : 'Открыть презентацию в приложении')}
        </button>
        <a
          class="button button--secondary"
          href="${escapeHtml(lesson.presentation.href)}"
          download="${escapeHtml(lesson.presentation.fileName || 'presentation.pptx')}"
        >
          ${escapeHtml(lesson.presentation.label ?? 'Скачать PPTX')}
        </a>
      `
    : '<span class="status-chip status-chip--muted">Материал пока не подключен</span>';

  return `
    <div class="material-shelf">
      <div class="section-title section-title--compact">
        <div>
          <p class="eyebrow">Презентация урока</p>
          <h3>Материалы и предпросмотр</h3>
        </div>
        <span class="badge">${hasPresentation ? 'PPTX' : 'Скоро'}</span>
      </div>
      <div class="material-shelf__row">
        ${presentationMarkup}
      </div>
      ${buildPresentationViewerState(lesson, previewState)}
      <div class="material-shelf__path">
        <span class="muted">Папка урока</span>
...
function buildPresentationViewerState(lesson, previewState) {
  if (!lesson.presentation?.href) {
    return `
      <div class="notice">
        Презентация еще не загружена. Раздел уже встроен и примет реальный файл, как только он появится в storage.
      </div>
    `;
  }
  ...
  if (previewState.loading) {
    return `
      <div class="presentation-viewer presentation-viewer--loading">
        <div class="notice notice--soft">${escapeHtml(PRESENTATION_PREVIEW_LOADING_TEXT)}</div>
      </div>
    `;
  }
    ...
    return `
      <div class="presentation-viewer">
        <div class="presentation-toolbar">
          <span class="status-chip status-chip--success">Превью активно</span>
          <span class="muted">Если превью не отображается, используйте кнопку скачивания.</span>
        </div>
        <div class="presentation-frame">
          <iframe
            src="${escapeHtml(previewState.viewerUrl)}"
            title="${escapeHtml(`Презентация ${lesson.title}`)}"
            loading="lazy"
          ></iframe>
        </div>
      </div>
  ...
  return `
    <div class="presentation-viewer">
      <div class="notice notice--soft">
        Нажмите кнопку выше, чтобы открыть презентацию внутри приложения.
      </div>
    </div>
  `;
}

function refreshPresentationPanel(lesson) {
  const container = document.querySelector('#presentationPanel');
  if (!container) {
    return;
  }

  container.innerHTML = buildPresentationPanel(lesson);
}

function bindPresentationActions(lesson) {
  const previewButton = document.querySelector('#openPresentationPreview');
  if (!previewButton) {
    return;
  }

  previewButton.addEventListener('click', async () => {
    if (!lesson.presentation?.previewEndpoint) {
      setPresentationPreviewState(lesson.id, {
        loading: false,
        viewerUrl: '',
        error: 'Не удалось подготовить встроенный маршрут для этой презентации.'
      });
    ...
    try {
      const response = await fetch(lesson.presentation.previewEndpoint, {
        headers: { Accept: 'application/json' }
      });
      const payload = await response.json().catch(() => ({}));

      if (!response.ok || !payload.viewerUrl) {
        throw new Error(payload.message || `Не удалось открыть презентацию (${response.status}).`);
      }
      ...
      setPresentationPreviewState(lesson.id, {
        loading: false,
        viewerUrl: '',
        error: error.message || 'Не удалось подготовить встроенный просмотр презентации.'
      });
    }
